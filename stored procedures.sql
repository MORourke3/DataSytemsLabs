...
-- Returns immediate prereqs of passed in course
create or replace function get_prereqs(int, REFCURSOR) returns refcursor as 
$$
declare
   num_course int       := $1;
   resultset   REFCURSOR := $2;
...
select get_prereqs (0, 'results');
Fetch all from results;


-- Returns courses for which passed in course is a prereq
create or replace function get_postreq(int, REFCURSOR) returns refcursor as 
$$
declare
   num_course int       := $1;
   resultset   REFCURSOR := $2;
